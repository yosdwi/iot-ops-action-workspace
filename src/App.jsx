...
const EMPTY_MASTERS = { sites: [], shifts: [], people: [], issueTypes: [], actionTypes: [], statuses: [], units: [] }
...
function Workspace({ session }) {
  ...
  useEffect(() => {
    loadTickets()
  }, [dateFrom, dateTo, siteFilter, shiftFilter, issueFilter, statusFilter, responderFilter])
  ...
  async function loadMasters() {
    const queries = await Promise.all([
      ...
      supabase.from('units').select('unit_id,unit_no,site_id').eq('is_active', true).order('unit_no').limit(5000),
      supabase.rpc('get_ticket_suggestions', { p_kind: 'issue_description', p_search: '', p_limit: 120 }),
      supabase.rpc('get_ticket_suggestions', { p_kind: 'corrective_action', p_search: '', p_limit: 120 }),
    ])
    const firstError = queries.find((x) => x.error)?.error
    if (firstError) return notify(firstError.message, 'error')
    setMasters({ sites: queries[0].data || [], shifts: queries[1].data || [], people: queries[2].data || [], issueTypes: queries[3].data || [], actionTypes: queries[4].data || [], statuses: queries[5].data || [], units: queries[6].data || [] })
    setSuggestions({ issueDescriptions: (queries[7].data || []).map((x) => x.value), correctiveActions: (queries[8].data || []).map((x) => x.value) })
  }
  ...
  async function saveTicketAction(row, patch = {}, solve = false) {
    if (!operatorId) return notify('Pilih operator IoT Ops dulu.', 'error')
    const before = { ...row }
